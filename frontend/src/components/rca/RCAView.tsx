...
export function RCAView() {
  ...
  const [anomalies, setAnomalies] =
    useState<AnomalyListResponse | null>(null);

  // Trigger Mode: "anomaly" | "manual"
  const [triggerMode, setTriggerMode] =
    useState<"anomaly" | "manual">("anomaly");

  const [selectedAnomalyId, setSelectedAnomalyId] =
    useState<string>("");

  // Manual Trigger Fields
  const [metricCol, setMetricCol] =
    useState<string>("");

  const [dateCol, setDateCol] =
    useState<string>("");

  const [aggregation, setAggregation] =
    useState<"sum" | "avg" | "count">("sum");
  ...
  const [baselineMode, setBaselineMode] =
    useState<"prior_period" | "custom">("prior_period");

  const [baselineStart, setBaselineStart] =
    useState<string>("");

  const [baselineEnd, setBaselineEnd] =
    useState<string>("");

  const [loading, setLoading] =
    useState(false);

  const [rcaResult, setRcaResult] =
    useState<RCAResponse | null>(null);
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((p) => {
        ...
        // IMPORTANT:
        // Always use the exact column names from the uploaded dataset.
        const actualColumns = p.column_names ?? [];

        const dateCols = p.columns
          .map((c, index) => ({
            profile: c,
            actualName:
              actualColumns[index] ?? c.name,
          }))
          .filter(
            ({ profile: c }) =>
        ...
        if (dateCols.length > 0) {
          setDateCol(
            dateCols[0].actualName
          );
        } else if (actualColumns.length > 0) {
          setDateCol(actualColumns[0]);
        }

        if (numCols.length > 0) {
          setMetricCol(
            numCols[0].actualName
          );
        }

        // Set default dates if available
        const dtStat =
          dateCols[0]?.profile.datetime_stats;
    ...
    api
      .listAnomalies(currentDataset.id)
      .then((res) => {
        setAnomalies(res);
  ...
  // If targetAnomalyId was set from AnomaliesView,
  // automatically execute analysis
  useEffect(() => {
  ...
  async function executeAnomalyRCA(
    anomalyId: string
  ) {
    if (!currentDataset || !anomalyId)
      return;
    ...
    try {
      ...
      if (triggerMode === "anomaly") {
        if (!selectedAnomalyId) {
          throw new Error(
            "Please select an anomaly record to analyze."
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Metric column, date column, and event start/end dates are required."
          );
        }

        res = await api.analyzeRCA(
          currentDataset.id,
          {
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500 max-w-sm">
          Please select or upload a dataset before
          performing waterfall root-cause analysis.
        </p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col justify-between gap-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-slate-900">
            Waterfall Root-Cause Analysis
          </h1>

          <p className="text-xs text-slate-500 mt-0.5">
            Deterministic driver decomposition, exact
            statistical z-scores, and 100% waterfall
            reconciliation.
          </p>
        </div>
            ...
            <div className="flex-1 max-w-md">
              ...
              >
                ...
                anomalies.anomalies.length >
                  ...
                  <option value="">
                    No persisted anomalies
                    available (run Anomaly
                    Detection first)
                  </option>
                )}
              </select>
      ...
      {rcaResult && (
        <div className="space-y-6">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="rounded-full bg-white px-2.5 py-0.5 text-xs font-semibold text-indigo-700 border border-indigo-200">
                    ...
                    {rcaResult.aggregation.toUpperCase()}
                    )
                  </span>
                </div>
            ...
            <div className="flex flex-col justify-between gap-4 border-b border-slate-100 pb-4 sm:flex-row sm:items-center">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  ...
                  pre-truncation segment drivers
                  reconciles to total metric change:
              ...
              <div className="text-right">
                ...
                >
                  ...
                  {rcaResult.total_delta.toLocaleString(
                    undefined,
                    {
                  ...
                  (
                  {rcaResult.percentage_change >
                  0
                    ? "+"
                    : ""}
                  {rcaResult.percentage_change.toFixed(
                    1
                  )}
                  %)
                </p>
            ...
            <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4 text-xs">
              <div className="rounded-lg bg-slate-50 p-3">
                ...
                <p className="font-bold text-slate-900 mt-0.5">
                  {rcaResult.event_window.start}{" "}
                  → {rcaResult.event_window.end}
                </p>
              </div>
              ...
              <div className="rounded-lg bg-slate-50 p-3">
                ...
                <p className="font-bold text-slate-900 mt-0.5">
                  {rcaResult.baseline_window.start}{" "}
                  →{" "}
                  {rcaResult.baseline_window.end}
                </p>
              </div>
              ...
              <div className="rounded-lg bg-slate-50 p-3">
                ...
                <p className="font-mono font-bold text-slate-900 mt-0.5">
                  {rcaResult.reconciliation.discrepancy.toFixed(
                    6
                  )}
                </p>
              </div>
          ...
          {/* Primary Ranked Drivers */}
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <div className="border-b border-slate-200 px-6 py-4">
              <h3 className="text-base font-bold text-slate-900">
                Ranked Segment Drivers
              </h3>

              <p className="text-xs text-slate-500">
                Segment contributions sorted by
                magnitude with statistical z-score
                significance:
                <span className="font-mono">
                  {" "}
                  z = (y_event - μ_base) / σ_base
                </span>
              </p>
            </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 font-mono">
                            {d.baseline_value.toLocaleString(
                              undefined,
                              {
                                maximumFractionDigits: 2,
                              }
                              ...
                              null ? (
                                <span className="font-mono font-semibold text-slate-800">
                                  ...
                                  {d
                                    ...
                                    ? "+"
                                    : ""}
                                  {d.statistical_score.z_score.toFixed(
                                    2
                                  )}
                                </span>
                              ) : (
          ...
          {/* Secondary Metric Co-Movement */}
          {rcaResult.secondary_metrics.length >
            0 && (
            <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
              <div className="border-b border-slate-200 px-6 py-4">
                <h3 className="text-base font-bold text-slate-900">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs text-slate-600">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {rcaResult.secondary_metrics.map(
                      (sec) => (
                        <tr
                          key={sec.metric_name}
                          className="hover:bg-slate-50"
                        >
                          ...
                          <td className="px-6 py-4 font-mono">
                            {sec.baseline_value.toLocaleString(
                              undefined,
                              {
                                maximumFractionDigits: 2,
                              }
                          ...
                          <td className="px-6 py-4 font-mono font-semibold text-slate-900">
                            {sec.event_value.toLocaleString(
                              undefined,
                              {
                                maximumFractionDigits: 2,
                              }
