...
type AnalyticsSubTab = "overview" | "time-series" | "breakdown" | "correlation";
...
export function AnalyticsView() {
  ...
  const [tsGranularity, setTsGranularity] = useState<string>("month");
  const [tsAggregation, setTsAggregation] = useState<string>("sum");
  const [tsRollingWindow, setTsRollingWindow] = useState<number | null>(null);
  const [tsGrowth, setTsGrowth] = useState<boolean>(false);
  const [tsCumulative, setTsCumulative] = useState<boolean>(false);
  ...
  const [bdAggregation, setBdAggregation] = useState<string>("sum");
  ...
  useEffect(() => {
    ...
    api.getProfile(currentDataset.id).then((p) => {
      ...
      const catCols = p.columns.filter(
        (c) => c.classification === "categorical" || c.inferred_datatype === "string"
      );

      if (dateCols.length > 0) setTsDateCol(dateCols[0].name);
      else if (p.columns.length > 0) setTsDateCol(p.columns[0].name);

      if (numCols.length > 0) {
        setTsMetricCol(numCols[0].name);
        setBdMetricCol(numCols[0].name);
        setCorrCols(numCols.slice(0, 8).map((c) => c.name));
      }
      if (catCols.length > 0) setBdDimension(catCols[0].name);
      else if (p.columns.length > 0) setBdDimension(p.columns[0].name);
    }).catch(console.error);
  }, [currentDataset?.id]);

  // Load Overview when tab changes to overview
  useEffect(() => {
    if (!currentDataset || activeTab !== "overview") return;
    setOverviewLoading(true);
    setError(null);
    api.getOverview(currentDataset.id)
      .then(setOverview)
  ...
  async function fetchTimeSeries() {
    if (!currentDataset || !tsDateCol || !tsMetricCol) return;
    setTsLoading(true);
    setError(null);
    try {
      const res = await api.queryTimeSeries(currentDataset.id, {
        date_column: tsDateCol,
        metric_column: tsMetricCol,
        granularity: tsGranularity,
        aggregation: tsAggregation,
        rolling_window: tsRollingWindow || undefined,
  ...
  async function fetchBreakdown() {
    if (!currentDataset || !bdDimension) return;
    setBdLoading(true);
    setError(null);
    try {
      const res = await api.queryBreakdown(currentDataset.id, {
        dimension: bdDimension,
        metric_column: bdMetricCol || undefined,
  ...
  async function fetchCorrelation() {
    if (!currentDataset || corrCols.length < 2) {
      setError("Please select at least 2 numeric columns for correlation.");
      return;
    }
    setCorrLoading(true);
    setError(null);
    try {
      const res = await api.queryCorrelation(currentDataset.id, {
        columns: corrCols,
        method: corrMethod,
      });
      setCorrResult(res);
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500 max-w-sm">
          Please select or upload a dataset to run aggregations, time-series, breakdowns, and correlation analysis.
        </p>
        <button
  ...
  const timeSeriesChartOption: echarts.EChartsOption = {
    ...
    xAxis: {
      type: "category",
      data: tsResult ? tsResult.series.map((p) => p.timestamp) : [],
    ...
    series: [
      {
        ...
        type: "line",
        smooth: true,
        data: tsResult ? tsResult.series.map((p) => p.value) : [],
        ...
        ? [
            {
              ...
              type: "line" as const,
              smooth: true,
              data: tsResult.series.map((p) => p.rolling_average ?? null),
        ...
        ? [
            {
              ...
              type: "line" as const,
              smooth: true,
              data: tsResult.series.map((p) => p.cumulative_value ?? null),
        ...
        ? [
            {
              ...
              type: "bar" as const,
              yAxisIndex: 1,
              data: tsResult.series.map((p) => p.growth_rate_pct ?? null),
              itemStyle: { color: "rgba(244, 63, 94, 0.6)" },
            },
          ]
        : []),
    ],
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col justify-between gap-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-slate-900">Multi-Dimensional Analytics</h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Interactive temporal time-series, segment share breakdowns, and Pearson/Spearman correlation matrices.
          </p>
        </div>
        ...
        <div className="flex items-center rounded-lg bg-slate-100 p-1 text-xs font-semibold">
          ...
          <button
            onClick={() => {
              ...
              if (!corrResult) fetchCorrelation();
            }}
            className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 transition ${
        ...
        <div className="space-y-6">
          {overviewLoading ? (
            <div className="py-16 text-center text-slate-500">
              ...
              <p className="text-sm font-semibold">Aggregating dataset overview KPIs...</p>
            </div>
          ) : overview ? (
            <>
              {/* Temporal KPI Banner */}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-900 mt-0.5">
                        {overview.temporal_kpi.min_date} — {overview.temporal_kpi.max_date}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">
                      {overview.temporal_kpi.range_days} Days Covered
                    </span>
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {Object.entries(overview.numeric_kpis).map(([col, stats]) => (
                    <div
                      key={col}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="rounded bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600">
                          {(stats?.count ?? 0).toLocaleString()} rows
                        </span>
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-xs border-t border-slate-100 pt-2">
                        <div>
                          ...
                          <p className="font-semibold text-slate-800">
                            {stats?.sum != null ? stats.sum.toLocaleString() : "—"}
                          </p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-slate-800">
                            {stats?.std != null ? stats.std.toFixed(2) : "—"}
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              {/* Top Dimensions */}
              {Object.keys(overview.top_dimension_kpis).length > 0 && (
                <div>
                  <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">
                    Top Categorical Dimensions
                  </h3>
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {Object.entries(overview.top_dimension_kpis).map(([dim, cats]) => (
                      <div key={dim} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                        ...
                        <div className="space-y-2">
                          {cats.slice(0, 5).map((cat) => (
                            <div key={cat.category} className="flex justify-between text-xs">
                              <span className="text-slate-700 truncate mr-2" title={cat.category}>
                                {cat.category || "(blank)"}
                              </span>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
              ...
              <div>
                ...
                <select
                  value={tsAggregation}
                  onChange={(e) => setTsAggregation(e.target.value)}
                  className="w-full rounded-md border border-slate-300 px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setTsRollingWindow(e.target.value ? Number(e.target.value) : null)}
                  className="w-full rounded-md border border-slate-300 px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none"
                >
              ...
              <div className="flex items-end">
                ...
                >
                  {tsLoading ? "Resampling..." : "Run Query"}
                </button>
              </div>
            </div>
            ...
            <div className="mt-3 flex items-center gap-6 border-t border-slate-100 pt-3 text-xs text-slate-600">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={tsGrowth}
                  onChange={(e) => setTsGrowth(e.target.checked)}
                  className="rounded text-indigo-600"
                />
                <span>Include Growth Rate %</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={tsCumulative}
                  onChange={(e) => setTsCumulative(e.target.checked)}
                  className="rounded text-indigo-600"
                />
                <span>Include Cumulative Total</span>
              </label>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 className="text-base font-bold text-slate-900 mb-2">
              Time Series: {tsAggregation.toUpperCase()} of {tsMetricCol} by {tsGranularity}
            </h3>
            {tsResult && tsResult.series.length > 0 ? (
              <ChartWrapper option={timeSeriesChartOption} height="400px" loading={tsLoading} />
            ) : (
              <div className="py-20 text-center text-slate-400">
                Click "Run Query" to visualize the metric time series
              </div>
            )}
          </div>
        </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
              <div>
                ...
                <select
                  value={bdDimension}
                  onChange={(e) => setBdDimension(e.target.value)}
                  className="w-full rounded-md border border-slate-300 px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none"
                >
                  {profile?.columns.map((c) => (
                    <option key={c.name} value={c.name}>
              ...
              <div>
                ...
                <select
                  value={bdMetricCol}
                  onChange={(e) => setBdMetricCol(e.target.value)}
                  className="w-full rounded-md border border-slate-300 px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none"
                >
              ...
              <div>
                ...
                <input
                  ...
                  max={50}
                  value={bdTopN}
                  onChange={(e) => setBdTopN(Number(e.target.value))}
                  className="w-full rounded-md border border-slate-300 px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <select
                  value={bdAggregation}
                  onChange={(e) => setBdAggregation(e.target.value)}
                  className="w-full rounded-md border border-slate-300 px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none"
                >
                  <option value="sum">Sum</option>
                  <option value="mean">Mean</option>
              ...
              <div className="flex items-end">
                <button
                  ...
                  className="w-full rounded-md bg-indigo-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-indigo-700 disabled:opacity-50"
                >
                  {bdLoading ? "Calculating..." : "Compute Share"}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="text-sm font-bold text-slate-900 mb-4">
                Share of Total: {bdDimension}
              </h3>
              {bdResult && bdResult.items.length > 0 ? (
                <ChartWrapper option={breakdownChartOption} height="360px" loading={bdLoading} />
              ) : (
                <div className="py-20 text-center text-slate-400">
                  Compute breakdown to view segment share
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm overflow-hidden">
              <h3 className="text-sm font-bold text-slate-900 mb-4">Segment Values</h3>
              {bdResult && bdResult.items.length > 0 ? (
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {bdResult.items.map((it) => (
                        <tr key={it.category} className="hover:bg-slate-50">
                          <td className="px-4 py-2 font-medium text-slate-800">{it.category}</td>
                          <td className="px-4 py-2 font-mono text-slate-600">
                            {it.value.toLocaleString()}
                          </td>
                          <td className="px-4 py-2 text-right font-bold text-indigo-600">
                            {it.percentage_of_total.toFixed(1)}%
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="py-20 text-center text-slate-400">No data computed</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                ...
                <div className="flex items-center gap-3 text-xs font-semibold">
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    <input
                      ...
                      name="corrMethod"
                      value="pearson"
                      checked={corrMethod === "pearson"}
                      onChange={() => setCorrMethod("pearson")}
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    <input
                      ...
                      name="corrMethod"
                      value="spearman"
                      checked={corrMethod === "spearman"}
                      onChange={() => setCorrMethod("spearman")}
                      className="text-indigo-600"
                    />
              ...
              >
                {corrLoading ? "Calculating Correlation..." : "Compute Correlation Matrix"}
              </button>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm overflow-x-auto">
            <h3 className="text-base font-bold text-slate-900 mb-4">
              Pairwise Correlation Heatmap ({corrMethod.toUpperCase()})
            </h3>
            {corrResult ? (
              <table className="w-full border-collapse text-center text-xs">
                <thead>
                  <tr>
                    <th className="p-2 text-left font-semibold text-slate-500">Metric</th>
                    {corrResult.columns.map((col) => (
                      <th key={col} className="p-2 font-mono font-semibold text-slate-700">
                        {col}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {corrResult.columns.map((rowCol) => (
                    <tr key={rowCol} className="border-t border-slate-100">
                      <td className="p-2 text-left font-mono font-semibold text-slate-800">
                        {rowCol}
                      </td>
                      {corrResult.columns.map((colCol) => {
                        const val = corrResult.correlation_matrix[rowCol]?.[colCol];
                        ...
                        if (val !== null && val !== undefined) {
                          if (val >= 0.7) {
                            ...
                            textColor = "text-indigo-900 font-semibold";
                          } else if (val > -0.3) {
                        ...
                        return (
                          <td key={colCol} className="p-1.5">
                            ...
                            >
                              {val !== null && val !== undefined ? val.toFixed(2) : "NaN"}
                            </div>
                          </td>
                        );
                      })}
              ...
              <div className="py-20 text-center text-slate-400">
                Click "Compute Correlation Matrix" to calculate pairwise correlation coefficients
              </div>
            )}
          </div>
        </div>
