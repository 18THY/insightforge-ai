...
export function ForecastsView() {
    ...
    api
      ...
      .then((p) => {
        ...
        const dateCols = p.columns
          .map((c, index) => ({
            profile: c,
            actualName: actualColumns[index] ?? c.name,
          }))
          .filter(
            ({ profile: c }) =>
        ...
        if (dateCols.length > 0) {
          setDateCol(dateCols[0].actualName);
        } else if (actualColumns.length > 0) {
          setDateCol(actualColumns[0]);
        }

        if (numCols.length > 0) {
          const actualMetric = numCols[0].actualName;

          setMetricCol(actualMetric);

          api
            .getForecasts(currentDataset.id, actualMetric)
            .then((res) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500 max-w-sm">
          Please select or upload a dataset before generating time-series
          forecasts.
        </p>
  ...
  const forecastChartOption: echarts.EChartsOption = {
    ...
    xAxis: {
      type: "category",
      data: forecastResult
        ? forecastResult.forecasts.map((f) => f.forecast_date)
        : [],
      axisLabel: {
        color: "#64748b",
        rotate: 30,
    ...
    series: [
      // Base line for confidence band
      {
        name: "Lower Bound Base",
        type: "line",
        data: forecastResult
          ? forecastResult.forecasts.map(
              (f) => f.lower_bound ?? f.predicted_value
            )
          : [],
        lineStyle: {
          opacity: 0,
      ...
      // Predicted line
      {
        ...
        smooth: true,
        data: forecastResult
          ? forecastResult.forecasts.map((f) => f.predicted_value)
          : [],
        itemStyle: {
          color: "#4f46e5",
        },
      ...
      <div className="flex flex-col justify-between gap-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Fit deterministic OLS linear trend + calendar seasonality models
            with mathematical prediction bands.
          </p>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <div>
            ...
            >
              {profile?.column_names.map((columnName) => (
          ...
          <div>
            ...
            <select
              value={cadence}
              onChange={(e) =>
                setCadence(e.target.value as "D" | "W" | "M")
              }
              className="w-full rounded-md border border-slate-300 px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none"
            >
          ...
          <div className="flex items-end">
            ...
            >
              ...
              <span>
                {loading ? "Fitting Model..." : "Generate Forecast"}
              </span>
            </button>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {forecastResult && forecastResult.forecasts.length > 0 ? (
          ...
          <div className="py-20 text-center text-slate-400">
            Configure horizon and click "Generate Forecast" to simulate
            forward trends and prediction intervals.
          </div>
        )}
      </div>
      ...
      {forecastResult && forecastResult.forecasts.length > 0 && (
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="border-b border-slate-200 px-6 py-4">
            <h3 className="text-base font-bold text-slate-900">
              Forecast Schedule & Confidence Intervals
            </h3>

            <p className="text-xs text-slate-500">
              Point estimates accompanied by 95% mathematical confidence
              boundaries
            </p>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-3 font-mono text-slate-600">
                        {pt.lower_bound !== null
                          ? pt.lower_bound.toLocaleString(undefined, {
                              maximumFractionDigits: 2,
                            })
                          : "—"}
                      </td>
                      ...
                      <td className="px-6 py-3 font-mono text-slate-600">
                        {pt.upper_bound !== null
                          ? pt.upper_bound.toLocaleString(undefined, {
                              maximumFractionDigits: 2,
                            })
                          : "—"}
                      </td>
                      ...
                      <td className="px-6 py-3 font-mono text-right text-slate-500">
                        {spread !== null
                          ? `± ${(spread / 2).toLocaleString(undefined, {
