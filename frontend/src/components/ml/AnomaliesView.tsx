...
export function AnomaliesView() {
  ...
  const [method, setMethod] =
    useState<"statistical" | "isolation_forest">("statistical");
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((p) => {
        ...
        // Use the exact column names from the uploaded dataset.
        ...
        if (numCols.length > 0) {
          ...
          api
            .listAnomalies(currentDataset.id, actualMetric)
            .then((res) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500 max-w-sm">
          Please select or upload a dataset before running
    ...
    series: [
      {
        ...
        smooth: true,
        data: anomalyResult
          ? anomalyResult.anomalies.map((a) => a.value)
          : [],
        itemStyle: {
          color: "#4f46e5",
        },
...

      {
        ...
        symbolSize: 12,
        data: anomalyResult
          ? anomalyResult.anomalies.map((a, idx) => [
              idx,
              a.value,
            ])
          : [],
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col justify-between gap-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Identify statistical outliers using rolling Z-Score
            windows or scikit-learn IsolationForest algorithms.
          </p>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between mb-2">
          ...
          {anomalyResult && (
            <span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-700">
              {anomalyResult.total_anomalies} Anomalies
              Found
            </span>
          )}
        </div>
        ...
        anomalyResult.anomalies.length > 0 ? (
          <ChartWrapper
            option={anomalyChartOption}
            height="360px"
            loading={loading}
          />
        ) : (
          <div className="py-20 text-center text-slate-400">
            Configure algorithm parameters above and click
            "Detect Anomalies" to inspect statistical
            deviations.
          </div>
        )}
      </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <div className="border-b border-slate-200 px-6 py-4">
              ...
              <p className="text-xs text-slate-500">
                Each flagged anomaly can be seamlessly
                investigated in the Root-Cause Analysis
                engine with 1 click.
              </p>
            </div>
